...
<%@page import="POJO.fotosPaginasBairrosPOJO"%>
<%@page import="DAO.paginasBairrosDAO"%>
<%@page import="BEAN.paginasBairrosBEAN"%>
<%@page import="DAO.paginasBairrosDAO"%>
<%@page import="DAO.fotosPaginasBairrosDAO"%>
<%@page import="DAO.paginasBairrosDAO"%>
<%@page import="BEAN.paginasBairrosBEAN"%>
<%@page import="Until.functions"%>
<%@page import="java.util.Vector"%>
<%@page import="DAO.fotosPaginasBairrosDAO"%>
<%@page import="BEAN.fotosPaginasBairrosBEAN"%>
    ...
    <body onload="relogio();">
        <center>
            <%
                ...
                fotosPaginasBairrosBEAN fotosPaginas_BEAN=new fotosPaginasBairrosBEAN();
                fotosPaginasBairrosDAO fotosPaginas_DAO=new fotosPaginasBairrosDAO(minhaConexao);
                ...
                String id_paginas_bairros=request2.getParameter("id_paginas_bairros");
                ...
                String mensagem_Src="";
                String mensagem_id_paginas_bairros="";          
                  
                if ((id_paginas_bairros==null)||((id_paginas_bairros!=null)&&(id_paginas_bairros.equals("null"))))
                    id_paginas_bairros=request.getParameter("id_paginas_bairros");
                
                try{fotosPaginas_BEAN.setID(ID);}catch(Exception erro){mensagem_ID="a foto selecionada não existe";};
                try{fotosPaginas_BEAN.setId_Paginas_Bairros(id_paginas_bairros);}catch(Exception erro){mensagem_id_paginas_bairros=erro.getMessage();}; 
                if((acao!=null)&&(acao.length()>0)){ 
                        ...
                        if(acao.equals("Buscar")){
                            fotosPaginasBairrosPOJO  fotosPaginas_POJO = new fotosPaginasBairrosPOJO();
                            fotosPaginas_POJO.setTitulo(Titulo);
                            fotosPaginas_POJO.setDescricao(descricao);
                            fotosPaginas_BEAN= new fotosPaginasBairrosBEAN(fotosPaginas_POJO);
                            ...
                            mensagem_id_paginas_bairros="";
                                ...
                                if (
                                    ...
                                    && 
                                    (mensagem_id_paginas_bairros.length()==0)                                                                                                                           
                                ){                             
                                    try{
                                        String acc="";
                                ...
                                fotosPaginas_BEAN=((fotosPaginasBairrosBEAN)fotosPaginas_DAO.buscarID(fotosPaginas_BEAN));
                                if((fotosPaginas_BEAN!=null)&&(fotosPaginas_BEAN.getIDStr()!=null)){
                                    registros_fotosPaginas.add(fotosPaginas_BEAN);
                                    mensagem_id_paginas_bairros="";
                                }
            ...
            <input type="hidden" name="id_categoria" value="<%=id_paginas_bairros%>">
               <div style="clear:both;color:red"><%=mensagem_id_paginas_bairros%></div>
                    ...
                    <%
                        if((registros_fotosPaginas!=null)&&(registros_fotosPaginas.size()>0)){
                            for(int i=0;i<registros_fotosPaginas.size();i++){
                                fotosPaginas_BEAN=((fotosPaginasBairrosBEAN)registros_fotosPaginas.get(i));
